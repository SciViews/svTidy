...
library('dplyr') # TODO: use library_dplyr() instead
library('collapse')
library('svTidy')
```
...
> {svTidy} provides functions equivalent to [{dplyr}](https://dplyr.tidyverse.org) and [{tidyr}](https://tidyr.tidyverse.org), but using either a standard evaluation of arguments or formulas, both being closer to base R syntax. Its function are also usually faster and consume less RAM.
...
We got a leaner code thanks to formulas in {svTidy} for many advanced features. We will see another example with **injection** (or **quasi-quotation**) later in a **more complex example**.

## Installation

You can install the development version of {svTidy} from the [SciViews R-unverse](https://sciviews.r-universe.dev) with:
...
Alternatively, you can also install it from [GitHub](https://github.com/) with:
...
Being still in experimental stage, it is not available yet on [CRAN](https://cran.r-project.org), but it will be submitted as soon as it matures.

## More complex example

The best way to appreciate {svTidy} is by comparing it to {dplyr} on a more complex example. Still using the `starwars` dataset, let's say we want to calculate the mean and standard deviation of the age of all humans, two years after the Battle of Yavin (note that `birth_year` is in years before that battle, so, you got it with `birth_date + 2` in years), grouped by their gender. We also want the number of non-missing observations in each group.
...
The {dplyr} version needs substantial edition of the code to include it in a function and exhibits ultimately a less readable result that {svTidy} that generalizes the code more smoothly. Here is the result of using these two functions:
...
ages_dplyr_res
```
Note that `ages_svTidy()` arguments somehow naturally "inherits" the properties of the underlying {svTidy} functions: NSE expressions are to be provided as formulas. Consistency and clarity is an important feature of the code!

Now, some more improvement in `summarise_()` can be done by using {collapse} **fast statistical functions**:
...
bm
```
With such a small dataset, we essentially measure the overhead of the different approaches, and we can see that {svTidy} (fast version) is `r round(as.numeric(bm$median[1] / bm$median[3]), 1)` times faster, and it requires `r round(as.numeric(bm$mem_alloc[1] / bm$mem_alloc[3]), 1)` times less memory than {dplyr} in this case. This is because {svTidy} uses a more lightweight mechanisms thanks to the formulas, and also (although of limited impact on small datasets) because it relies on efficient {collapse} or {data.table} code to do the computation. Of course, result vary with the functions used and the dataset size.
