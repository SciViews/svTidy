...
test_that("filter_() works with standard evaluation", {
  data(mtcars)
  result <- filter_(mtcars, mtcars$mpg > 20, mtcars$cyl == 4)
...
test_that("filter_() works with multiple grouping variables", {
  ...
  expect_true(is_grouped_df(result))
  expect_equal(length(group_vars_(result)), 2)
})
...
test_that("slice_head_() with prop parameter", {
  ...
  expected_n <- floor(nrow(mtcars) * 0.1)
...
test_that("slice_tail_() with prop parameter", {
  ...
  expected_n <- floor(nrow(mtcars) * 0.2)
  expect_equal(nrow(result), expected_n)
})
...
test_that("slice_() works with negative indices", {
  ...
  expect_false(all(rownames(mtcars)[1:3] %in% rownames(result)))
})
